...
# labeling with KNN for comments
labelCommentsWithRandomForest <- function(project, comments) {
  bugCommentsHigh = read.csv(paste(project, "/bug_comments_thresholds_high.csv", sep=""))
  bugCommentsLow  = read.csv(paste(project, "/bug_comments_thresholds_low.csv", sep=""))
  bugComments = data.frame(nocomm=bugCommentsHigh[,1])
  for( i in 1:nrow(bugComments) ) {
    if( bugCommentsHigh[i,2]==0 ) {
      bugComments$class[i] = "low"
    }
  ...
  bugComments$class = as.factor(bugComments$class)
  rf = randomForest(class ~ ., data=bugComments)
  comments$state = predict(rf, comments)
  return(comments)
}

# labeling with KNN for contributions
labelContributionsWithRandomForest <- function(project, contributions) {
  contributionsHigh = read.csv(paste(project, "/contribution_thresholds_high.csv", sep=""))
  contributionsLow  = read.csv(paste(project, "/contribution_thresholds_low.csv", sep=""))
  contributionsPreLabeled = data.frame(dnoc=contributionsHigh[,1], bugfixes=contributionsLow[,2])
  for( i in 1:nrow(contributionsPreLabeled) ) {
  ...
  contributionsPreLabeled$class = as.factor(contributionsPreLabeled$class)
  rf = randomForest(class ~ ., data=contributionsPreLabeled)
  contributions$state = predict(rf, contributions)
  return(contributions)
}

# labeling with KNN for communication
labelCommunicationWithRandomForest <- function(project, communication) {
  communicationHigh = read.csv(paste(project, "/communication_thresholds_sum_high.csv", sep=""))
  communicationLow  = read.csv(paste(project, "/communication_thresholds_sum_low.csv", sep=""))
  communicationPreLabeled = data.frame(activities=communicationHigh[,1])
  for( i in 1:nrow(communicationPreLabeled) ) {
    if( communicationHigh[i,2]==0 ) {
      communicationPreLabeled$class[i] = "low"
    }
  ...
  communicationPreLabeled$class = as.factor(communicationPreLabeled$class)
  rf = randomForest(class ~ ., data=communicationPreLabeled)
  communication$state = predict(rf, communication)
  return(communication)
}
...
corAndKL <- function(model1, model2, project1, project2, type) {
  ...
  exitCode = try({
    klDivLow = kl.norm(model1$parms.emission$mu[[1]], model1$parms.emission$sigma[[1]], model2$parms.emission$mu[[1]], model2$parms.emission$sigma[[1]])
    klDivMedium = kl.norm(model1$parms.emission$mu[[2]], model1$parms.emission$sigma[[2]], model2$parms.emission$mu[[2]], model2$parms.emission$sigma[[2]])
    klDivHigh = kl.norm(model1$parms.emission$mu[[3]], model1$parms.emission$sigma[[3]], model2$parms.emission$mu[[3]], model2$parms.emission$sigma[[3]])
    #print(paste("  KL divergence between project", project1, "and", project2, "for", type, "models and state low:", klDivLow))
    #print(paste("  KL divergence between project", project1, "and", project2, "for", type, "models and state medium:", klDivMedium))
    #print(paste("  KL divergence between project", project1, "and", project2, "for", type, "models and state high:", klDivHigh))
  }, silent=TRUE)
  if( class(exitCode)=="try-error" ) {
    print(paste("  failure during KL divergence calculation between projects ", project1, "and", project2))
  }
}
