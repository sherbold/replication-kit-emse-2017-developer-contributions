######################################################
# DEFINE THE LOCATION OF THE REPLICATION KIT HERE!!! #
# This is the only line you have to change.          #
######################################################
setwd("C:/SWESVN/Papers/Journals/DeveloperContribution/data_emse")

# Check and install dependencies
if(!require(HMM)) install.packages("HMM")
if(!require(lattice)) install.packages("lattice")
if(!require(ggplot2)) install.packages("ggplot2")
if(!require(mhsmm)) install.packages("mhsmm")
...
projects = c("k3b","amarok","ant", "egit", "konsole", "log4j", "poi","rekonq")

coreDevelopers <- c("dev1","dev2","dev3", "dev4","dev5","dev6", "dev7","dev111","dev112")
majorDevelopers <- c("dev8","dev9","dev10","dev11","dev12","dev13","dev14","dev15","dev16","dev17","dev18","dev19", "dev20", "dev21", "dev22", "dev23", "dev24", "dev25", "dev26", "dev27", "dev28", "dev29", "dev30", "dev31", "dev32", "dev33","dev34","dev35","dev36","dev37","dev38","dev39","dev40","dev41","dev42","dev43","dev44","dev45","dev46", "dev110", "dev47","dev113","dev114","dev115","dev67","dev116","dev117","dev124")
minorDevelopers <- c("dev48","dev49","dev50","dev51","dev52","dev53","dev54","dev55", "dev56", "dev57", "dev58", "dev59", "dev60", "dev61", "dev62", "dev63", "dev64", "dev65","dev66","dev67","dev68","dev121","dev70","dev71","dev72","dev73","dev118","dev119","dev86","dev120")

initClassifiers = c("thresholder", "knn", "knn3", "knn5", "rf")
...
for( projectIndex in 1:length(projects)) {
  ...
  for( classifierIndex in 1:length(initClassifiers) ) {
    ...
    else if( initClassifier=="rf" ) {
      comments = labelCommentsWithRandomForest(project, comments)
      communication = labelCommunicationWithRandomForest(project, communication)
      contributions = labelContributionsWithRandomForest(project, contributions)
    }
    else {
...
for( projectIndex in 1:length(projects)) {
  ...
  for( classifierIndex in 1:length(initClassifiers) ) {
    ...
    for(devIndex in 1:length(developers)) {
      developerName = developers[devIndex]
      print(paste("    calculating error for developer", developerName))
      ...
      classObsAll <- rbind(classObs,classObsAll)
...
if( CLEANUP ) {
  rm(contributions, communication, comments, classObs)
  rm(thr_comments, thr_communication, thr_contributions)
  rm(knn_comments, knn_communication, knn_contributions)
  rm(diff_comments, diff_communication, diff_contributions)
...
summary_knn3 = summary[summary$initClassifier=="knn3",]
summary_knn5 = summary[summary$initClassifier=="knn5",]
summary_rf = summary[summary$initClassifier=="rf",]
...
summary$errorUnivKnn3 = summary_knn3$errorUniv
summary$errorDevKnn3 = summary_knn3$errorDev
summary$errorUnivKnn5 = summary_knn5$errorUniv
summary$errorDevKnn5 = summary_knn5$errorDev
summary$errorUniRF = summary_rf$errorUniv
summary$errorDevRF = summary_rf$errorDev

if( CLEANUP ) {
  rm(summary_thresholder, summary_knn, summary_knn3, summary_knn5, summary_rf)
}

print(colMeans(summary[,4:ncol(summary)], na.rm=TRUE))

# 7) Simulate HMM for example application
...
# 7.1) Simulate the HMM for one core, 2 major, and 5 minor devs
...
print(ggplot(commits, aes(x=commits$month, y=commits$value, colour=commits$variable, group=commits$variable)) 
      ...
      + theme(text = element_text(size=15),axis.text.x = element_text(hjust = 1, size=15),axis.text.y = element_text(hjust = 1, size=15),
              legend.text = element_text(size = 15))
      + guides(color=guide_legend(title="developer type")))


# 7.3) Create plots for states
...
# 7.4.2) Boxplots
boxplotdata <- summary[,c(2,5,7,9,11,13)]

colnames(boxplotdata)[2] <- "thresholder"
colnames(boxplotdata)[3] <- "knn"
colnames(boxplotdata)[4] <- "knn3"
colnames(boxplotdata)[5] <- "knn5"
colnames(boxplotdata)[6] <- "random forest"
...
boxplotuniversal <- summary[,c(2,4,6,8,10,12)]

colnames(boxplotuniversal)[2] <- "universal thresholder"
colnames(boxplotuniversal)[3] <- "universal knn"
colnames(boxplotuniversal)[4] <- "universal knn3"
colnames(boxplotuniversal)[5] <- "universal knn5"
colnames(boxplotuniversal)[6] <- "universal random forest"
...
if( CLEANUP ) {
  rm(nSim, month, states, commits)
  rm(boxplotdata, p, boxplotuniversal)
  rm(core, major, major1, major2, minor, minor1, minor2, minor3, minor4, minor5)
  rm(simCore, simMajor1, simMajor2, simMinor1, simMinor2, simMinor3, simMinor4, simMinor5)
  rm(outputTotal)
}

# 8) Create information about data distribution

# 8.1) Number of developers in low, medium, high state

for (i in 1:nrow(classObsAll)) {
  if (classObsAll$name[i] %in% coreDevelopers ) {
    classObsAll$type[i] <- "core"
  }
  else if (classObsAll$name[i] %in% majorDevelopers ) {
    classObsAll$type[i] <- "major"
  }
  else if (classObsAll$name[i] %in% minorDevelopers ) {
    classObsAll$type[i] <- "minor"
  }
  else {
    classObsAll$type[i] <- "minor"
  }
...
classObsAll$stateLabel <- factor(classObsAll$stateLabel, levels = c("low", "medium","high"))

# 8.2) Plot information about emissions

coreModels_knn5 <- list()
for (i in (1:length(coreModels))) {
  if (coreModels[[i]]$initClassifier=="knn5") {
    coreModels_knn5 <- rbind(coreModels_knn5,coreModels[[i]])
  }
}

means = data.frame("project"=character(),"low"=double(),"medium"=double(),"high"=double(),stringsAsFactors=FALSE)
for (i in (1:nrow(coreModels_knn5))) {
  means <- rbind(means,data.frame(project=coreModels_knn5[i,]$project,low=coreModels_knn5[i,]$model$parms.emission$mu[[1]][1],
                 medium=coreModels_knn5[i,]$model$parms.emission$mu[[2]][1],high=coreModels_knn5[i,]$model$parms.emission$mu[[3]][1]))
}
means$type = "core"
means$activity = "commits"

means2 = data.frame("project"=character(),"low"=double(),"medium"=double(),"high"=double(),stringsAsFactors=FALSE)
for (i in (1:nrow(coreModels_knn5))) {
  means2 <- rbind(means2,data.frame(project=coreModels_knn5[i,]$project,low=coreModels_knn5[i,]$model$parms.emission$mu[[1]][2],
                                  medium=coreModels_knn5[i,]$model$parms.emission$mu[[2]][2],high=coreModels_knn5[i,]$model$parms.emission$mu[[3]][2]))
}
means2$type = "core"
means2$activity = "bug fixes"

means3 = data.frame("project"=character(),"low"=double(),"medium"=double(),"high"=double(),stringsAsFactors=FALSE)
for (i in (1:nrow(coreModels_knn5))) {
  means3 <- rbind(means3,data.frame(project=coreModels_knn5[i,]$project,low=coreModels_knn5[i,]$model$parms.emission$mu[[1]][3],
                                    medium=coreModels_knn5[i,]$model$parms.emission$mu[[2]][3],high=coreModels_knn5[i,]$model$parms.emission$mu[[3]][3]))
}
means3$type = "core"
means3$activity = "ML posts"

means4 = data.frame("project"=character(),"low"=double(),"medium"=double(),"high"=double(),stringsAsFactors=FALSE)
for (i in (1:nrow(coreModels_knn5))) {
  means4 <- rbind(means4,data.frame(project=coreModels_knn5[i,]$project,low=coreModels_knn5[i,]$model$parms.emission$mu[[1]][4],
                                    medium=coreModels_knn5[i,]$model$parms.emission$mu[[2]][4],high=coreModels_knn5[i,]$model$parms.emission$mu[[3]][4]))
}
means4$type = "core"
means4$activity = "bug comments"


means <- rbind(means,means2,means3,means4)
means <- means[,c(2,3,4,6)]
means <- melt(means,id=c("activity"))

p <- ggplot(data=means, aes(x=factor(means$activity),y=means$value,fill=means$variable))
print(p + geom_boxplot() + xlab("activity") + ylab("number")
      + scale_y_continuous(limits = c(0,100))
      + theme(text = element_text(size=15),axis.text.x = element_text(hjust = 0.5, size=15),axis.text.y = element_text(hjust = 1, size=15),
              legend.text = element_text(size = 15))
...
if( CLEANUP ) {
  rm(means, means2, means3, means4)
  rm(coreModels_knn5)
  rm(p, i)
}

# 9) Create correlatoin plots

mu1 <- hmmCore$parms.emission$mu[[1]][,1]
Sigma1 <-  hmmCore$parms.emission$sigma[[1]]
mvn1 <- mvrnorm(5000, mu = mu1, Sigma = Sigma1 )
mu2 <- hmmCore$parms.emission$mu[[2]][,1]
Sigma2 <-  hmmCore$parms.emission$sigma[[2]]
mvn2 <- mvrnorm(5000, mu = mu2, Sigma = Sigma2 )
mu3 <- hmmCore$parms.emission$mu[[3]][,1]
Sigma3 <-  hmmCore$parms.emission$sigma[[3]]
mvn3 <- mvrnorm(5000, mu = mu3, Sigma = Sigma3 )

mu4 <- hmmMajor$parms.emission$mu[[1]][,1]
Sigma4 <-  hmmMajor$parms.emission$sigma[[1]]
mvn4 <- mvrnorm(5000, mu = mu4, Sigma = Sigma4 )
mu5 <- hmmMajor$parms.emission$mu[[2]][,1]
Sigma5 <-  hmmMajor$parms.emission$sigma[[2]]
mvn5 <- mvrnorm(5000, mu = mu5, Sigma = Sigma5 )
mu6 <- hmmMajor$parms.emission$mu[[3]][,1]
Sigma6 <-  hmmMajor$parms.emission$sigma[[3]]
mvn6 <- mvrnorm(5000, mu = mu6, Sigma = Sigma6 )

mu7 <- hmmMinor$parms.emission$mu[[1]][,1]
Sigma7 <-  hmmMinor$parms.emission$sigma[[1]]
mvn7 <- mvrnorm(5000, mu = mu7, Sigma = Sigma7 )
mu8 <- hmmMinor$parms.emission$mu[[2]][,1]
Sigma8 <-  hmmMinor$parms.emission$sigma[[2]]
mvn8 <- mvrnorm(5000, mu = mu8, Sigma = Sigma8 )
mu9 <- hmmMinor$parms.emission$mu[[3]][,1]
Sigma9 <-  hmmMinor$parms.emission$sigma[[3]]
mvn9 <- mvrnorm(5000, mu = mu9, Sigma = Sigma9 )

par(mfrow=c(3,3)) 

corrplot(cor(mvn1), method="ellipse",tl.pos="n")
text(2.5,5, "core low", col="red", cex=1)
corrplot(cor(mvn2), method="ellipse",tl.pos="n")
text(2.5,5, "core medium", col="red", cex=1)
corrplot(cor(mvn3), method="ellipse",tl.pos="n")
text(2.5,5, "core high", col="red", cex=1)
corrplot(cor(mvn4), method="ellipse",tl.pos="n")
text(2.5,5, "major low", col="red", cex=1)
corrplot(cor(mvn5), method="ellipse",tl.pos="n")
text(2.5,5, "major medium", col="red", cex=1)
corrplot(cor(mvn6), method="ellipse",tl.pos="n")
text(2.5,5, "major high", col="red", cex=1)
corrplot(cor(mvn7), method="ellipse",tl.pos="n")
text(2.5,5, "minor low", col="red", cex=1)
corrplot(cor(mvn8), method="ellipse",tl.pos="n")
text(2.5,5, "minor medium", col="red", cex=1)
corrplot(cor(mvn9), method="ellipse",tl.pos="n")
text(2.5,5, "minor high", col="red", cex=1)

if( CLEANUP ) {
  rm(mvn1, mvn2, mvn3, mvn4, mvn5, mvn6, mvn7, mvn8, mvn9)
  rm(mu1, mu2, mu3, mu4, mu5, mu6, mu7, mu8, mu9)
  rm(Sigma1, Sigma2, Sigma3, Sigma4, Sigma5, Sigma6, Sigma7, Sigma8, Sigma9)
}
